...
(function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			div = element("div");
    			svg = svg_element("svg");
    			attr_dev(svg, "id", "holder");
    			add_location(svg, file, 63, 2, 1545);
    			attr_dev(div, "id", "innerGraphicHolder");
    			add_location(div, file, 62, 0, 1513);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	let { used, total } = $$props;
    	let svgElement;

    	onMount(() => {
    		let dims = { width: 300, height: 300, margins: 20 };

    		// chop off useless sig figs
    		$$invalidate(1, used = parseFloat(used.toFixed(1)));

    		$$invalidate(2, total = parseFloat(total.toFixed(1)));
    		let svg = select(svgElement);
    		...
    		let pie$1 = pie()([used, total - used]);
    		console.log(pie$1);
    		let arcgen = arc().innerRadius(70).outerRadius(100);

    		g.selectAll(".graph").data(pie$1).enter().append("path").attr("class", "graph").attr("fill", (d, i) => {
    			if (i == 0) {
    				// purplish
    				return "black";
    			}
    			...
    			return "white";
    		}).attr("stroke", "black").attr("d", arcgen);

    		g.selectAll("text").data(pie$1).enter().append("text").attr("transform", d => {
    		...
    		}).style("text-anchor", "middle").text(d => `${d.data}TB`).attr("fill", "black").attr("class", "storageText");
    	});

    	const writable_props = ['used', 'total'];
    	...
    	$$self.$$set = $$props => {
    		if ('used' in $$props) $$invalidate(1, used = $$props.used);
    		if ('total' in $$props) $$invalidate(2, total = $$props.total);
    	};

    	$$self.$capture_state = () => ({ d3, onMount, used, total, svgElement });
    	...
    	return [svgElement, used, total, svg_binding];
    }

    class App extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init$1(this, options, instance, create_fragment, safe_not_equal, { used: 1, total: 2 });
    		...
    		if (/*used*/ ctx[1] === undefined && !('used' in props)) {
    			console_1.warn("<App> was created without expected prop 'used'");
    		}

    		if (/*total*/ ctx[2] === undefined && !('total' in props)) {
    			console_1.warn("<App> was created without expected prop 'total'");
    		}
    	}
